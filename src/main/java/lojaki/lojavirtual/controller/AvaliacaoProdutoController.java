...
import lojaki.lojavirtual.model.AvaliacaoProduto;
import lojaki.lojavirtual.repository.AvaliacaoProdutoRepository;
...
public class AvaliacaoProdutoController {
	...
	public ResponseEntity<AvaliacaoProduto> salvarAvaliacaoProduto(@RequestBody @Valid AvaliacaoProduto avaliacaoProduto) throws ExceptionLojaki{
		...
		if(avaliacaoProduto.getProduto() == null || (avaliacaoProduto.getProduto() != null && avaliacaoProduto.getProduto().getId() <= 0)) {
			 throw new ExceptionLojaki("A avaliação deve conter o produto associado.");
		}
		
		
		if(avaliacaoProduto.getPessoa() == null || (avaliacaoProduto.getPessoa() != null && avaliacaoProduto.getPessoa().getId() <= 0)) {
			 throw new ExceptionLojaki("A avaliação deve conter a pessoa ou cliente associado.");
		}
