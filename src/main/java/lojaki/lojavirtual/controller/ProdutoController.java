...
public class ProdutoController {
	...
	public ResponseEntity<Produto> salvarProduto(@RequestBody @Valid Produto produto) throws ExceptionLojaki, MessagingException, IOException {

		if (produto.getTipoUnidade() == null || produto.getTipoUnidade().trim().isEmpty()) {
			throw new ExceptionLojaki("Tipo da unidade deve ser informada");
		}
		
		if (produto.getNome().length() < 4) {
			throw new ExceptionLojaki("Nome do produto deve ter mais de 4 caracteres.");
		}
		...
		if (produto.getQuantidadeEstoque() < 1) {
			throw new ExceptionLojaki("O produto dever ter no minímo 1 no estoque.");
		}
		
		if (produto.getImagens() == null || produto.getImagens().isEmpty() || produto.getImagens().size() == 0) {
			throw new ExceptionLojaki("Deve ser informado imagens para o produto.");
		}
		
		if (produto.getImagens().size() < 3) {
			throw new ExceptionLojaki("Deve ser informado pelo menos 3 imagens para o produto.");
		}
		
		if (produto.getImagens().size() > 6) {
			throw new ExceptionLojaki("Deve ser informado no máximo 6 imagens.");
		}
